...
__global__ 
void z_funct(Uint32 *d_img, cplx_plan_struct * d_cplx_plan, int nb_ite)
{
  int i = blockDim.x * blockIdx.x + threadIdx.x;
  int j = blockDim.y * blockIdx.y + threadIdx.y;

  double c_re = d_cplx_plan->min_re + 
    (d_cplx_plan->max_re-d_cplx_plan->min_re)
    /d_cplx_plan->nb_pts_re*i;

  double c_im = d_cplx_plan->max_im - 
    (d_cplx_plan->max_im-d_cplx_plan->min_im)
    /d_cplx_plan->nb_pts_im*j;
...
//    if ( t < nb_ite/2) b = 256-2*floor(256./nb_ite*t);
    ...
    //r = floor(256./nb_ite*t);
    //g = floor(256./nb_ite*(nb_ite-2*abs(nb_ite/2.-t)));
    //b = 256-floor(256./nb_ite*t);
  }
  d_img[j*d_cplx_plan->nb_pts_re+i] = r*65536+g*256+b; 
}
...
/*    printf( "Help \n 
  ...
  dim3 gridSize (ceil((float) nb_pts_re/16), ceil((float) nb_pts_im/16));
  ...
  SDL_Surface *SDL_img = SDL_SetVideoMode(nb_pts_re, nb_pts_im, 32, SDL_HWSURFACE | SDL_DOUBLEBUF);
  SDL_Event event;
  ...
  while ( !quit )
  {
        ...
        case SDL_KEYDOWN:
          switch ( event.key.keysym.sym )
          {
            ...
            case SDLK_i:
              ...
              break;
            case SDLK_s :
              SDL_SaveBMP(SDL_img, "data_mandelbrot.bmp");
              break;
            default:
              break;
          }
    ...
    if (refresh)
    {
      if (recalc)
      {
        ...
        z_funct<<<gridSize,blockSize>>>(d_img, d_cplx_plan, nb_ite);
      ...
      //for(int i=0; i<size_i; i++) printf(" %f", *(SDL_img->pixels+i));
      SDL_Flip(SDL_img);
    }
  }
